...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 22 Jan 2020 pada 16.20
...
-- Prosedur
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `impilisitJoin` ()  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `innerJoin` ()  NO SQL
BEGIN
SELECT pl.id_pelanggan, nama, tgl_transaksi, total_transaksi
FROM pelanggan pl
INNER JOIN penjualan pn ON pl.id_pelanggan = pn.id_pelanggan;
-- yg ditampilin pl 1 sisa yg di join
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Join-use` ()  NO SQL
begin
SELECT pl.id_pelanggan, nama, tgl_transaksi, total_transaksi
FROM pelanggan pl
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `leftOuterJoin` ()  NO SQL
BEGIN
SELECT pl.id_pelanggan, nama, tgl_transaksi, total_transaksi
FROM pelanggan pl
LEFT OUTER JOIN penjualan pn ON pl.id_pelanggan = pn.id_pelanggan;
-- yg ditampilin pl 1 sisa yg di join
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `rightOuterJoin` ()  NO SQL
BEGIN
SELECT pl.id_pelanggan, nama, tgl_transaksi, total_transaksi
FROM pelanggan pl
RIGHT OUTER JOIN penjualan pn ON pl.id_pelanggan = pn.id_pelanggan;
-- yg ditampilin pl 1 sisa yg di join
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `x_lanjutan1` ()  NO SQL
SELECT pl.id_pelanggan, nama, id_transaksi, tgl_transaksi, total_transaksi
FROM pelanggan pl
LEFT JOIN penjualan pn USING (id_pelanggan)
WHERE pl.id_pelanggan = 2 OR pl.id_pelanggan = 1$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `x_lanjutan2` ()  NO SQL
SELECT pl.id_pelanggan, nama, id_transaksi, tgl_transaksi, total_transaksi
FROM pelanggan pl, penjualan pn
WHERE pl.id_pelanggan = pn.id_pelanggan
